...
function BillerSkuIo(props) {
    ...
    const headerProps = {
        headerTitle: `${t('PAY_BILL.PAY')}  ${data?.billerType} ${(slug !== 'mobile_prepaid' || slug !== 'top-up') ? t('PAY_BILL.PAY') : ''}` ,
        headerRight: true,
    };
                        ...
                        {({handleChange, values, handleSubmit, errors, setFieldValue}) => {
                            return (
                                <View style={[AuthStyle.formContainer, {flexGrow:1}]}>
                                    ...
                                    <CButton title={t('GLOBAL.NEXT')} loading={reduxState.loading} disabled={reduxState.loading} onPress={() => handleSubmit()}/>
