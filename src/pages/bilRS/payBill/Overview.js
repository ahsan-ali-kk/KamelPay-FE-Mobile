...
function Overview(props) {
        ...
        if(res?.error){
            ...
            if(resData?.hasLowBalance) {
                ...
                trackTransaction(payload, {
                    ...resData,
                    referenceNo,
                    vendor: topupAndPaybillmentVendor['BILRS'].id,
    ...
    const trackTransaction = (payload, res) => {
        let receiverDetail = {
            ...
            hasLowBalance: res?.hasLowBalance
        };
    ...
    const getTotalAmount = () => {
        ...
        let c = data?.billInfo?.type ==='DIRECT_PAY' ? data?.sku?.ReceivingCurrency : data?.payloadSend?.Currency;
        return {
    ...
    const generateTransferAmountAndCharges = () => {
        ...
        let c = obj?.billInfo?.type ==='DIRECT_PAY' ? obj?.sku?.ReceivingCurrency : obj?.payloadSend?.Currency;

        // if(obj?.billInfo?.BaseCurrency) {
        if(obj?.billInfo?.SettlementCurrency) {
            ...
            info.push({
                ...
                value: obj?.billInfo?.TotalAmountDue ? `${formatAmount(obj?.billInfo?.BillAmountDue)} ${obj?.billInfo?.BaseCurrency}`
                    :
                    `${formatAmount(obj?.sku?.Amount)} ${obj?.sku?.ReceivingCurrency}`
            })
        }
        ...
        if(obj?.billInfo?.type ==='AMOUNT_IN_SKU') {
            info.push({
                Name: t('RECEIPT.AMOUNT_DUE'),
                value: `${formatAmount(obj?.sku?.Amount)} ${obj?.sku?.ReceivingCurrency}`
            })
        }
