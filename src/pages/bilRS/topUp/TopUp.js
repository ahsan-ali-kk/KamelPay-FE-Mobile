...
import {
    billerConstant,
    getMobileCarrierLookup,
    getPostpaidBillers,
    getTopUpBeneficiary
} from "../../../store/actions/TopUp.action";
...
function TopUp(props) {
    ...
    const getMobileCarrier = (item) => {
        if(item.biller?.BillerType?.includes('Postpaid' || 'Mobile Postpaid')){
            let da = {
                biller: item?.biller,
    ...
    const renderItem = ({item}) => {
        let iconUrl = `${iconBilrsBaseUrl}/${item?.biller?.CountryCode}/${convertToSlug(item?.biller?.BillerID)}.png`;
    ...
    const horizontalListOnPress = (item) => {
        if(item.BillerType?.includes(billerConstant[0]) || item.BillerType?.includes(billerConstant[1]) || item.BillerType?.includes(billerConstant[2]) && item?._id == 'INTERNATIONAL' ) {
            navigation.navigate('add_mobile_topup', {
                item,
                ...(paramData?.moduleType && {moduleType: paramData?.moduleType})
            })
        } else if(item.BillerType?.includes(billerConstant[2])) {
            getSku(item)
        }
    };

    const sequenceData = (array = []) => {
        let sequence = billerConstant;
        let sequenceData = [];
        sequence?.forEach(o => {
                    ...
                    renderElement={(item, index) => {
                        return(
                            <HorizontalList
                                ...
                                renderImageFunc={(val) => {
                                    if(val?.BillerName?.toLowerCase()?.includes('du')){
                                        return require('../../../assets/images/du.png')
                                    } else if(val?.BillerName?.toLowerCase()?.includes("etisalat")) {
                                ...
                                renderTitleFunc={(val) => {
                                   return val ? val?.BillerName.replace(/prepaid|postpaid/gi, "").trim() : val
                                }}
