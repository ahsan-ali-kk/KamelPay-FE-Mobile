...
import {CButton, CInput, ProgressiveImage, CText, IconButton} from "../../uiComponents";
...
function Pay(props) {
    ...
    const headerProps = {
        headerTitle: `${t('PAY_BILL.PAY')} ${data?.billerType} ${slug !== 'mobile_prepaid' ? t('PAY_BILL.BILL') : ''}` ,
        headerRight: true,
    };
    ...
    const submit = (values) => {
        ...
        let payloadData = {
            ...updatedPayload,
            Currency: billInfo?.SettlementCurrency || sku.Currency,
    ...
    useEffect(() => {
        if(flag && (billInfo?.type === 'CUSTOM_AMOUNT') || (payload?.type === 'SOME_CONDITIONS')) {
            ...
            if(billInfo?.type === 'CUSTOM_AMOUNT') {
                amount = amount.min(Number(sku?.MinAmount || 1), `${t('VALIDATION.AMOUNT')} ${t('VALIDATION.AT_LEAST')} ${sku?.MinAmount || 1}.`);
                if(sku?.MaxAmount) {
                    amount = amount.max(Number(sku?.MaxAmount), `${t('VALIDATION.AMOUNT')} ${t('VALIDATION.NOT_MORE_THAN')} ${sku?.MaxAmount} ${t('VALIDATION.AMOUNT').toLowerCase()}.`)
                }
            } else if(payload?.type === 'SOME_CONDITIONS') {

                if(sku?.PartialPaymentAllowed || sku?.ExcessPaymentAllowed){
                    updateCustomPay(true)
                }

                if(sku?.PartialPaymentAllowed && sku?.ExcessPaymentAllowed){
                    let minA = sku?.MinAmount || 1;
                    amount = amount.min(minA, `${t('VALIDATION.AMOUNT')} ${t('VALIDATION.AT_LEAST')} ${minA}.`)
                } else if(sku?.PartialPaymentAllowed) {
                    let minA = sku?.MinAmount || 1;
                    let maxA = billInfo?.BillAmountDue;
                    amount = amount.min(Number(minA), `${t('VALIDATION.AMOUNT')} ${t('VALIDATION.AT_LEAST')} ${Number(minA)}.`);
                    amount = amount.max(Number(maxA), `${t('VALIDATION.AMOUNT')} ${t('VALIDATION.AT_LEAST')} ${Number(maxA)}.`);
                } else if(sku?.ExcessPaymentAllowed){
                    amount = amount.min(Number(billInfo?.BillAmountDue), `${t('VALIDATION.AMOUNT')} ${t('VALIDATION.AT_LEAST')} ${billInfo?.BillAmountDue}.`);
                    if(sku?.MaxAmount) {
                        amount = amount.max(Number(sku?.MaxAmount), `${t('VALIDATION.AMOUNT')} ${t('VALIDATION.NOT_MORE_THAN')} ${sku?.MaxAmount} ${t('VALIDATION.AMOUNT').toLowerCase()}.`)
                    }
                }
                                ...
                                <View style={Styles.container}>
                                    ...
                                    <View style={Styles.listPay}>
                                        ...
                                        {billInfo?.BaseCurrency ? renderListItem(
                                            ...
                                            billInfo?.TotalAmountDue ? `${formatAmount(billInfo?.BillAmountDue)} ${billInfo?.SettlementCurrency}` : `${formatAmount(sku?.Amount)} ${sku?.Currency}`, 'center')
                                            : null}
                                        {billInfo?.type ==='AMOUNT_IN_SKU' ? renderListItem(t('RECEIPT.AMOUNT_DUE'), `${formatAmount(sku?.Amount)} ${sku?.Currency}`, 'center') : null}
                                        {billInfo?.type ==='DIRECT_PAY' ? renderListItem(t('RECEIPT.AMOUNT'), `${formatAmount(sku?.Amount)} ${sku?.Currency}`, 'center') : null}
